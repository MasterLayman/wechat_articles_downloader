...
class ArticleDownloader:
    def __init__(self):
        ...
        # 获取按钮
        self.btn_get = ttk.Button(self.frame_top, text='获取', command=self.get_article_url)
        ...
        # 日志窗口（左侧）
        self.log_left = tk.Text(self.frame_bottom, font=('Arial', 12), wrap='none', state='disabled')
        self.log_left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        # 日志窗口（右侧）
        self.log_right = tk.Text(self.frame_bottom, font=('Arial', 12), wrap='none', state='disabled')
        self.log_right.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        # 滚动条相关设置
    ...
    def download_article(self):
        ...
        def download_thread():
            try:
                ...
                self.log_configure_and_insert("地址为：{}的文章下载失败".format(url))

        # 创建新线程并启动
        t = threading.Thread(target=download_thread)
        t.start()
        self.log_right.after(1000, functools.partial(self.log_heartbeat(t, url)))

    def log_heartbeat(self, t, url):
        if t.is_alive():
            self.log_configure_and_insert("地址为：{}的文章正在下载".format(url), "right")
            self.log_right.after(1000, functools.partial(self.log_heartbeat(t, url)))
    ...
    def log_configure_and_insert(self, log_text, window="left"):
        """
        在日志窗口中输出日志信息
        ...
        if window == "left":
            ...
            self.log_left.configure(state='normal')
            self.log_left.insert(tk.END, log_text + '\n')
            self.log_left.configure(state='disabled')
        if window == "right":
            # 设置文本框的状态为可编辑状态 打印日志 再次设为不可编辑
            self.log_right.configure(state='normal')
            self.log_right.insert(tk.END, log_text + '\n')
